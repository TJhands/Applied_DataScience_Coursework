...
def store_hpi_sales():
    ...
    for year in range(1998,2020):
        indices_ = indices.loc[indices['Date'].apply(lambda x: True if x >= f'{year}-01-01' and x <= f'{year}-12-01' else False)]
        sales_ = sales.loc[sales['Date'].apply(lambda x: True if x >= f'{year}-01-01' and x <= f'{year}-12-01' else False)]
...
if __name__ == '__main__':
    store_homelessness_england_year()